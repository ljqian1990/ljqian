...
{	
	...
	*/
	 ...
	 *      
	 *      $where = ['id'=>['or'=>['in'=>[1,2]]], 'name'=>['or'=>['like'=>'%2%']], 'time1'=>['like'=>'%2017-07-26%'], 'time2'=>['or'=>'2017-07-26 17:17:59']];
	 *      解析后条件为："OR `id` IN (1,2)  OR `name` LIKE '%2%' AND `time1` LIKE '%2017-07-26%' OR `time2`='2017-07-26 17:17:59'"
	 * 
	 ...
	 * @param unknown $value       需要自增的数值
	 * @param unknown $where       参见select函数的where用法
	 */
	public function incr($table, $field, $where=[], $value=1)
	{
	    $field = $this->_parseField($field);
	    $value = (int)$value;
	    $sql = sprintf("UPDATE %s SET %s=%s+%d", $table, $field, $field, $value);
	    if (!empty($where)) {
	 ...
	 * @param unknown $table       表名
	 * @param unknown $field       字段名
	 * @param unknown $value       需要自减的数值
	 * @param unknown $where       参见select函数的where用法
	 */
	public function decr($table, $field, $where=[], $value=1)
	{
	    $field = $this->_parseField($field);
	    $value = (int)$value;
	    $sql = sprintf("UPDATE %s SET %s=%s-%d", $table, $field, $field, $value);
