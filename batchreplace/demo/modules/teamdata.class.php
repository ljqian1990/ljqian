...
class PlgDemoTeamdata implements IPlugin
{

    const MEMCACHE_TEAMDATA_KEY = 'balls_demo_teamdata';

    const MEMCACHE_TEAMDATA_FORMVP_KEY = 'act_demo_teamdata_for_mvp';
    
    const MEMCACHE_TEAMDATASUM_KEY = 'balls_demo_teamdatasum';
    ...
    public function index()
    {
        ...
        $model = new CDemoteamdata($this->site_id);
     ...
     * }
     *
     * header('Location: ?model=teamdata&plugin=Demo&action=index');
     * }
     */
    public function import()
    {
        ...
        $smodel = new CDemoteamdatasum($this->site_id);
        ...
        foreach ($teamList as $teamname => $team) {
            foreach ($team['player'] as $player) {
                ...
                $this->replaceData('demo_teamdatatotal', $mb, $model);
            }
            ...
            $this->replaceData('demo_teamdatasum', $mbs, $smodel);
        }
        
        // 更新teamdatasum表，结算小组积分
        $ssql = 'SELECT A.`teamid`,A.`id` FROM demo_teamdatasum as A left join (select teamid,sum(`score`) as sumscore from demo_teamdatasum group by `teamid` order by sumscore desc) as B on A.teamid=B.teamid WHERE changci =' . $changci . ' ORDER BY  `score` DESC,`sumscore` DESC';
        ...
        $mvpmodel = new CDemomvp($this->site_id);
        ...
        $this->replaceData('demo_mvp', $md, $mvpmodel);
        
        // 同步teamdata表，队伍数据
        $sql = 'select `playerid`,ceil(AVG(`score`)) as per_score,MAX(`maxscore`) as max_maxscore,ceil(AVG(`eatnum`)) as per_eatnum,ceil(AVG(`dispersenum`)) as per_dispersenum,ceil(AVG(`deadnum`)) as per_deadnum,SUM(`mvpnum`) as sum_mvpnum from `demo_teamdatatotal` GROUP BY `playerid`';
        $ret = $model->doQueryList($sql);
        if (! empty($ret)) {
            $datamodel = new CDemoteamdata($this->site_id);
            foreach ($ret as $val) {
                $datamodel->updateBy([
                    'score' => $val['per_score'],
    ...
    private function setPlayerlist()
    {
        if (empty($this->playerlist)) {
            $model = new CDemoteamember($this->site_id);
