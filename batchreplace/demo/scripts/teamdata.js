...
var Teamdata = (function ($)
{
    ...
    this.init = function ()
    {
        ...
        $('table.ui-table span.ui-xicon').click(function ()
        {
            ...
            $.ajax({
            	url:'?model=teamdata&plugin=demo&action=active',
                data: {
                    'site_id': site_id,
                    'id': $(me).attr('bid')
                },
    ...
    this.xform = function (nid)
    {
        ...
        po_data.action = 'form';
        $.ajax({
        	url:'?plugin=demo&model=teamdata&action=form',
            data: po_data,
            dataType:"json",
            success: function (rp)
            {
    ...
    {       
        $.ajax({
        	url:'?model=teamdata&plugin=demo&action=save',
            data: $(f).serialize(),
            success: function (rp)
            {
                var txt = '';
    ...
    {
    	$.ajax({
    		'url' : '?model=teamdata&plugin=demo&action=clearmc',
    		success : function(rp) {
    			if (rp.returnflag != 'OK')
                {
    ...
    this.del = function (di)
    {
        ...
        $.ajax({
        	url:'?model=teamdata&plugin=demo&action=del',
            data: {
                'site_id': site_id,
                'del_id': data
            },
