...
class UploadFile
{
    ...
    public $thumb = false;
    // 图库类包路径
    public $imageClassPath = 'Image.php';
    // 缩略图最大宽度
    public $thumbMaxWidth = 126;
    // 缩略图最大高度
    public $thumbMaxHeight = 86;
    // 缩略图前缀
    ...
    private function save(&$file)
    {
        ...
        if ($this->thumb && in_array(strtolower($file['extension']), array(
            ...
            if (false !== $image) {
                ...
                // $this->thumbPath = $this->thumbPath?$this->thumbPath:$file['savepath'].dirname($file['savename']);
                ...
                for ($i = 0, $len = count($thumbWidth); $i < $len; $i ++) {
                    ...
                    $file['thumbfile'][] = Image::thumb($filename, $thumbname, '', $thumbWidth[$i], $thumbHeight[$i], true);
                }
    ...
    public function upload($savePath = '')
    {
        ...
        foreach ($files as $key => $file) {
            ...
            if (! empty($file['name'])) {
                ...
                // 复制一份完整的文件路径
                $file['imagefile'] = $file['savepath'] . $file['savename'];
                // 上传成功后保存文件信息，供其他地方调用
    ...
    private function check($file)
    {
        ...
        if (! $this->checkSize($file['size'])) {
            $this->error = '上传文件大小不能超过 ' . byte_format($this->maxSize);
            return false;
        }
        
        // 检查文件Mime类型
        if (! $this->checkType($file['type'])) {
            $this->error = '您好，系统只支持JPG,JPEG,GIF,PNG类型的图片';
            return false;
        }
        // 检查文件类型
