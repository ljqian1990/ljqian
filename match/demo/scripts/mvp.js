...
var Mvp = (function ($)
{
    ...
    this.xform = function (nid)
    {
        ...
        po_data.action = 'form';
        $.ajax({
        	url:'?plugin=ydmatch&model=mvp&action=form',
            data: po_data,
            dataType:"json",
            success: function (rp)
            {
    ...
    this.save = function (f)
    {
        ...
        $.ajax({
        	url:'?model=mvp&plugin=ydmatch&action=edit',
            data: $(f).serialize(),
            success: function (rp)
            {
                var txt = '';
    ...
    this.del = function (di)
    {
        ...
        $.ajax({
        	url:'?model=mvp&plugin=ydmatch&action=del',
            data: {
                'site_id': site_id,
                'del_id': data
            },
    ...
    {
        window.location.href = "/index.php?model=mvp&plugin=ydmatch&site_id=" + site_id + "&type=" + $(':input.srtype').val() + "&keyword=" + $(':input.srkey').val() + "&from=" + $(':input.srfrom').val() + "&to=" + $(':input.srto').val();
    };
    ...
    this.chg = function (oid, osort, nid, nsort)
    {
        ...
        $.ajax({
        	'url':'?model=mvp&plugin=ydmatch&action=zort',
            'data': {
                'site_id': site_id,
                'oid': oid,
                'osort': osort,
    ...
    {
    	$.ajax({
    		'url' : '?model=mvp&plugin=ydmatch&action=clearmc',
    		success : function(rp) {
    			if (rp.returnflag != 'OK')
                {
