...
var Teamember = (function ($)
{
    ...
    this.init = function ()
    {
        ...
        $('table.ui-table span.ui-xicon').click(function ()
        {
            ...
            $.ajax({
            	url:'?model=teamember&plugin=ydmatch&action=active',
                data: {
                    'site_id': site_id,
                    'id': $(me).attr('bid')
                },
    ...
    {
        if ($('div._teamember_infoForm').length > 0)
        {
            return false;
        }
        delete po_data.id;
        ...
        po_data.action = 'form';
        $.ajax({
        	url:'?plugin=ydmatch&model=teamember&action=form',
            data: po_data,
            dataType:"json",
            success: function (rp)
            {
                var winform = $('<div class="_teamember_infoForm">' + rp.html + '</div>').appendTo('body');
                winform.dialog({
                    title: '新闻信息',
                    width: 920,
                    position: [50, 50],
    ...
    this.save = function (f)
    {
        ...
        $.ajax({
        	url:'?model=teamember&plugin=ydmatch&action=edit',
            data: $(f).serialize(),
            success: function (rp)
            {
                var txt = '';
    ...
    this.del = function (di)
    {
        ...
        $.ajax({
        	url:'?model=teamember&plugin=ydmatch&action=del',
            data: {
                'site_id': site_id,
                'del_id': data
            },
    ...
    {
        window.location.href = "/index.php?model=teamember&plugin=ydmatch&site_id=" + site_id + "&type=" + $(':input.srtype').val() + "&keyword=" + $(':input.srkey').val() + "&from=" + $(':input.srfrom').val() + "&to=" + $(':input.srto').val();
    };
    ...
    this.chg = function (oid, osort, nid, nsort)
    {
        ...
        $.ajax({
        	'url':'?model=teamember&plugin=ydmatch&action=zort',
            'data': {
                'site_id': site_id,
                'oid': oid,
                'osort': osort,
    ...
    {
        var strhash = $('div._teamember_infoForm form').serialize();
    ...
    {
    	$.ajax({
    		'url' : '?model=teamember&plugin=ydmatch&action=clearmc',
    		success : function(rp) {
    			if (rp.returnflag != 'OK')
                {
